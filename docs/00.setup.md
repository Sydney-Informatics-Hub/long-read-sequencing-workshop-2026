...
3. By default, VS Code will suggest your home folder (`/home/<USERNAME>`), which is the directory we want to open. Press `Enter` to open VS Code in your home directory.
    - If for some reason VS Code is suggesting a different path than your home folder, you can use the drop-down box to navigate to it manually, or replace the path in the text box with `~` and press `Enter` (VS Code should automatically replace the `~` with the full path to your home folder, i.e. `/home/<USERNAME>`)
...
5. To open a terminal, type `Ctrl + J` (Windows) or `Cmd + J` (MacOS)
    - **TIP:** You can also use `` Ctrl + ` `` (the backtick key, all systems) to open a terminal, and `` Ctrl + Shift + ` `` to open additional terminal tabs, which show up in a panel on the right-hand side of the terminal view.

### Getting around the workspace and running commands

Run the three checks below in the terminal to confirm that everything you need is in place.

1. Check that the AMRFinderPlus database exists by listing its folder:

    ```bash
    ls ~/data/ref/amrfinderplus_db/
    ```

    You should see `2026-08-07.1` and `latest`.

2. Check that the sample assigned to your breakout room is present. For example, if your sample is `ERR8282752`:

    ```bash
    ls /home/<USERNAME>/data/fastqs/ERR8282752.fastq.gz
    ```

    You should see the file name displayed, and **not** a `No such file or directory` message.

3. Check that the software is installed by asking FastQC for its version:

    ```bash
    fastqc -v
    ```

    You should see `FastQC v0.12.1`.

If you encounter any issues with these checks, let the organisers know.
...
| Open folder | `Ctrl + K` then `Ctrl + O` | `Cmd + O` |
