...
![](./assets/05.ref_alignment_vs_de_novo_assembly.svg)
...
=== "Slightly branched graph"
    ...
    ![](./assets/05.bandage_plot.branched.svg)
...
=== "Heavily branched graph"
    ...
    ![](./assets/05.bandage_plot.bad.svg)
...
=== "raven"
    ...
    A typical `raven` run will look like:
    ...
    - `--identity <num>`: Threshold for overlap between two reads (default is `0`, i.e. no threshold)
    ...
    - **Number and length of contigs**: a tool like [`seqkit`](https://bioinf.shenwei.me/seqkit/) makes this easy:
        ...
        This reports the number of contigs along with their minimum, maximum, and average lengths, amongst other statistics. Alternatively, the `seqkit fx2tab` command will give you per-contig length information:
...
??? success "Solution"
    ...
    raven \
        ...
        --threads 4 \
        ~/filtered/sampleXX.filtered.fastq.gz > raven/assembly.fasta
    ...
    canu \
        ...
        genomeSize=5m \
        maxThreads=4 \
        -nanopore ~/filtered/sampleXX.filtered.fastq.gz
    ```
