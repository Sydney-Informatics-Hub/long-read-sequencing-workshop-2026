...
- `--plus` — also report the "Plus" subset (virulence factors, stress-response genes, etc.) alongside the default "Core" set. This training's pipeline always includes it — the biocide, stress, and virulence genes covered in Outputs (Section 3.1.2) only show up with `--plus` turned on.
...
- `--coverage_min <0-1>` — minimum proportion of the reference gene that must be covered for a BLAST-based hit (default `0.5`) — this is the 50% floor referenced in the Method column description (Section 3.1.2.2).
...
!!! info What can more coverage buy you?
...
* **Are the matches full-length or partial?** Mostly full-length: `qnrB35`, `sul1`, `blaIMP-1`, `aac(6')-Ib4`, and `qnrA1` are all clean `ALLELEX`/`EXACTX` calls at 100% coverage and identity. Two aren't — `blaCMY` is `INTERNAL_STOP` at 95.8% coverage (the same locus discussed in the coverage info box above), and `qnrA` is `PARTIALX` at just 61.9% coverage.
...
1. Run AMRFinderPlus on your polished assembly, including `--plus` so biocide, stress, and virulence genes are captured too (as covered in Outputs, Section 3.1.2).
...
??? solution Solution
    Here's how to reproduce the report and the gene-cassette plots:
    ...
    python3 "${HOME}/scripts/utilities/amr_cassette_diagram.py" \
        --amrfinder-tsv "amrfinder/${sample_id}.amrfinder_plus.tsv"

    # Combined assembly graph, so you can see how the contigs these genes sit
    # on actually connect (chromosome vs. individual plasmids)
    ...
    | Antibiotic classes (AMR genes) | EFFLUX, BETA-LACTAM (carbapenem), AMINOGLYCOSIDE, SULFONAMIDE | EFFLUX, BETA-LACTAM, AMINOGLYCOSIDE | BETA-LACTAM (cephalosporin + carbapenem), EFFLUX, FOSFOMYCIN, AMINOGLYCOSIDE, PHENICOL |
    ...
    **3. Which antibiotic classes are represented?** See the table above — ERR8282753 spans the most classes (5) despite having the fewest AMR genes overall; ERR8282751 has the fewest distinct classes (3) but the deepest gene count within them (an ESBL, extended-spectrum β-lactamase, plus two aminoglycoside genes on one plasmid).

    **4. Are any of the resistance genes on mobile elements that could support spread between bacteria or between species?** Only ERR8282751's: its entire plasmid_1 carries an ESBL (`blaCTX-M`) plus `blaOXA` and two aminoglycoside genes — a genuine multidrug-resistance plasmid. ERR8282742's and ERR8282753's AMR genes are all chromosomal, including, for ERR8282753, `blaOXA-78` ("OXA-51 family carbapenem-hydrolyzing class D beta-lactamase" per its Element name) — a full-length `ALLELEX` chromosomal hit.
    ...
    **6. Are there any efflux pump or stress-response genes that could contribute to reduced susceptibility or persistence?** Yes for all three, but at very different scale. ERR8282751 has by far the most: a 7-gene copper operon, a 6-gene silver operon, arsenic-resistance genes, acid-resistance genes, and three separate efflux pumps. ERR8282742 has mercury, cadmium, and copper resistance genes plus two efflux systems (`mexE`, and the `ttgRAB` cluster). ERR8282753 has the smallest stress footprint: a nickel-resistance gene and a 6-gene heat-resistance island (on its own plasmid, separate from its chromosomal AMR genes).
    ...
    **8. Does this sample suggest a potential outbreak risk based on the presence of mobile AMR genes?** ERR8282751 is the clearest case: an ESBL plus two aminoglycoside genes stacked on one plasmid is a classic transferable multidrug-resistance signature — though the ESBL call itself looks degraded here and would be worth confirming before relying on it. ERR8282742 carries a carbapenemase (`blaIMP-15`) but it's chromosomally integrated rather than plasmid-borne in this assembly — still outbreak-relevant given its likely class 1 integron context (a mobile gene cassette held together by its own recombination sites, not the plasmid it happens to sit on), just via a different mobility route than a typical plasmid-mediated spread. ERR8282753 shows the least plasmid association of the three: its carbapenemase (`blaOXA-78`) is chromosomal rather than plasmid-borne.
