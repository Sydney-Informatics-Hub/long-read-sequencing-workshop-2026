...
- `--plus` — also report the "Plus" subset (virulence factors, stress-response genes, etc.) alongside the default "Core" set. This training's pipeline always includes it — the biocide, stress, and virulence genes covered in the Outputs section below only show up with `--plus` turned on.
...
1. Run AMRFinderPlus on your polished assembly, including `--plus` so biocide, stress, and virulence genes are captured too (as covered in Outputs above).
