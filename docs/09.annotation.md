...
Type and Subtype describe *what kind* of element was found; Class and Subclass describe *what it's relevant to*. Definitions below follow the AMRFinderPlus wiki's own [Element Type and Subtype](https://github.com/ncbi/amr/wiki/Interpreting-results#element-type-and-subtype) table, annotated with which of these actually turn up in this training's own AMRFinderPlus reports for `ERR8282742`, `ERR8282751`, `ERR8282752`, and `ERR8282753`:

![](assets/09.annotation/type_subtype_diagram.png)

This covers every Type/Subtype combination that actually appears across the four samples above. Two Subtypes from the wiki table are missing entirely: `POINT`/`POINT_DISRUPT` and `STX_TYPE`/`ANTIGEN` don't show up because none of these four samples were run with `-O`/`--organism` (point mutation and StxTyper screening both require it) — not something wrong with the data, just outside what our `amrfinder` command in this training screens for. A fifth Subtype, `AMR-SUSCEPTIBLE`, is left out too, since it exists only in AMRFinderPlus's underlying Reference Gene Catalog and is never actually emitted in report output.

- **Class** — the drug class (or, for `STRESS` genes, the compound class) the gene is known to affect. Across these four reports: `BETA-LACTAM`, `AMINOGLYCOSIDE`, `SULFONAMIDE`, and `QUINOLONE` for `AMR` genes; `MERCURY`, `ARSENIC`, `COPPER`, `SILVER`, `NICKEL`, `CADMIUM`, and `CHROMATE` for `STRESS`/`METAL` genes; `QUATERNARY AMMONIUM` and `EFFLUX` for `STRESS`/`BIOCIDE` genes.
- **Subclass** — narrows Class further when known. Sometimes it's identical to Class, as for `qacEdelta1` (`ERR8282742`): Class `QUATERNARY AMMONIUM`, Subclass `QUATERNARY AMMONIUM`, no further detail available. Sometimes it's narrower — `blaIMP-15` (`ERR8282742`) is Class `BETA-LACTAM` but Subclass `CARBAPENEM` specifically, which matters clinically since not every beta-lactamase is a carbapenemase. And sometimes it lists several drugs the gene affects, slash-separated: `aac(6')-Ib3` (`ERR8282742`) is Class `AMINOGLYCOSIDE`, Subclass `AMIKACIN/KANAMYCIN/TOBRAMYCIN`.

!!! info Biocides
    Biocides are antimicrobial chemical agents used heavily across domestic, industrial, and healthcare settings for disinfection — disinfectants, antiseptics, and preservatives including quaternary ammonium compounds (QACs), biguanides, chlorine-releasing agents, and peroxygens. Their heavy use is itself a source of selective pressure: bacteria can evolve biocide resistance much as they evolve antibiotic resistance, which is exactly what `STRESS`/`BIOCIDE` genes such as `qacEdelta1` (seen in `ERR8282742`'s own report) confer.

    ![](assets/09.annotation/biocide.png)
    *How a QAC disinfectant kills a Gram-negative cell: it inserts into the membrane bilayer and disrupts it, releasing the autolytic enzymes that finish the cell off. An efflux pump like QacEdelta1 works by pumping the QAC back out before it can do this. Source: [Jones & Joshi, 2021 — "Biocide Use in the Antimicrobial Era: A Review"](https://www.mdpi.com/1420-3049/26/8/2276)*
