...
**Quality filtering** — enabled by default (`-Q` / `--disable_quality_filtering` turns it off). A read is discarded if too high a proportion of its bases are "unqualified": `-q` / `--qualified_quality_phred` sets what counts as a qualified base (default Q15), and `-u` / `--unqualified_percent_limit` sets how much of a read is allowed to fall below that before it's dropped (default 40%).
...
**Adapter and primer trimming.** By default, `dorado basecaller` looks for adapter and primer sequences at the start and end of each read and trims them from the output. Which sequences it searches for depends on the sequencing kit used, read from metadata embedded in the POD5 files — note that by default only kit14 sequencing kits are supported for this, so older or non-standard kits won't get any adapter/primer trimming. This is controlled with `--trim`, which takes one of three values:

| `--trim` value | Adapters | Primers | Barcodes |
|---|---|---|---|
| `all` *(default)* | trimmed | trimmed | trimmed, if barcoding is enabled |
| `adapters` | trimmed | not trimmed | not trimmed |
| `none` | not trimmed | not trimmed | not trimmed |
