...
- **`plassembler_plasmids.fasta`**: the final, assembled plasmid contigs, with each header annotated with its estimated copy number. An empty file will be created if no plasmids are detected.
- **`plassembler_plasmids.gfa`**: the corresponding assembly graph, viewable in [Bandage](https://rrwick.github.io/Bandage/) just like a `flye` assembly graph.
- **`plassembler_summary.tsv`**: a per-contig summary table giving each plasmid's length, estimated copy number, circularity, and (where one exists) its closest match in PLSDB.
...
Run `plassembler` using your assigned FASTQ file and the `flye` assembly you generated in [Lesson 5](./05.assembly.md). Place the output in a new directory called `plassembler` and use 4 CPU threads. Use the pre-downloaded plassembler database located at `~/data/ref/plasmid_db_plassembler`. Also be sure to pass the full path to your trimmed and filtered FASTQ file.
...
??? success "Solution"
    ...
    plassembler long \
        -l ~/filtered/sampleXX.filtered.fastq.gz \
    ...
    Remember to substitute your sample name for `sampleXX`. This assumes your `flye` outputs are in the `~/flye` directory, as per the instructions in the [Lesson 5 exercise](./05.assembly.md#531-run-flye).

Once finished, you should have a `plassembler` directory that looks like:

```
plassembler/
├── flye_output/
├── logs/
├── plassembler_1789366321.7878132.log
...
The `flye_output` and `unicycler_output` folders contain the intermediate results from those two tools. The `plassembler_plasmids.fasta` and `plassembler_plasmids.gfa` files contain the sequences and sequence graph, respectively (if any plasmids were found). `plassembler_summary.tsv` is the summary table describing each plasmid identified.

### 6.3.2 Inspect the outputs

Once `plassembler` has finished, inspect the summary table. Note that the table has a lot of columns, so open it in the VS Code editor. You can either navigate to it using the Explorer side-bar, or use the `code` command:

```bash
code plassembler/plassembler_summary.tsv
```
...
Using `flye/assembly_info.txt` and `plassembler/plassembler_summary.tsv`, work out which small contigs from each tool most plausibly represent the same underlying plasmid (matching on approximate length is usually enough), and which contigs are unique to one tool or the other.
...
- Check whether it has a PLSDB hit ([see below](#6331-checking-contigs-against-plsdb))
