...
The `flye_output` and `unicycler_output` folders contain the intermediate results from those two toosl. The `plassembler_plasmids.fasta` and `plassembler_plasmids.gfa` files contain the sequences and sequence graph, respectively (if any plasmids were found). `plassembler_summary.tsv` is the summary table describing each plasmid identified.
