...
In this exercise, you will run QUAST, BUSCO, and Bandage on the complete draft assembly you put together at the end of the [Lesson 6 exercise](./06.plasmids.md#634-create-your-complete-assembly) (your `flye` chromosome plus any additional plasmids recovered by `plassembler`), and use each tool's output to judge how complete and reliable that assembly is.

### 7.3.1 Run QUAST

Run QUAST on your draft assembly FASTA file. Place the output in a new directory called `assembly_qc/quast/draft`, and use 4 CPU threads.
...
??? success "Solution"
    ...
    ```bash
    mkdir -p assembly_qc/quast/draft
    ```

    Then, run QUAST on the draft assembly:
...
Once QUAST has finished, you should have the following folder structure in `assembly_qc`:
...
Look at the human-readable report:

```bash
code assembly_qc/quast/draft/report.txt
```
...
Run BUSCO on your draft assembly FASTA file using the pre-downloaded bacterial lineage database located at `~/data/ref/busco/bacteria_odb12.2`. Tell BUSCO to use 4 CPU threads and place the output in a new directory called `assembly_qc/busco/draft`. **Note: don't create the output directory yourself.** BUSCO will create if for you, and will refuse to run if the directory already exists.

??? success "Solution"

    ```bash
    busco \
        --in draft_assembly.fasta \
...
Once BUSCO has finished, you should see the following folder structure:
...
│       ├── short_summary.specific.bacteria_odb12.2.draft.json
│       └── short_summary.specific.bacteria_odb12.2.draft.txt
└── quast/
```
...
Visualise your assembly graph(s) with Bandage. **Note** that you can't run this on the draft assembly FASTA file; instead, you will have to use the individual `.gfa` graph files produced by `flye` and `plassembler`.

Place the output images into a folder called `assembly_qc/bandage`.

!!! warning "Harmless warning message"

    When you run Bandage, you may see a warning message like:

    ```
    QStandardPaths: error creating runtime directory '/run/user/1002' (No such file or directory)
    ```

    This doesn't affect the tool's output and can be safely ignored for our purposes.
...
??? success "Solution"
    ...
    ```bash
    mkdir -p assembly_qc/bandage
    ```

    Now, run bandage, first on the `flye` assembly:

    ```bash
    Bandage image \
        flye/assembly_graph.gfa \
        assembly_qc/bandage/flye_assembly_graph.svg
    ```
    ...
    Bandage image \
        ...
        assembly_qc/bandage/plassembler_plasmids_graph.svg
    ```

```
assembly_qc/
├── bandage
│   ├── sampleXX.flye_assembly_graph.svg
│   └── sampleXX.plassembler_plasmids_graph.svg
├── busco/
└── quast/
```
