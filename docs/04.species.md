...
![](./assets/04.kraken2_classification.jpg)
*Kraken read classification algorithm. Source: [Wood and Salzberg, 2014. **Kraken: ultrafast metagenomic sequence classification using exact alignments.** Genome Biol. 2014 Mar 3;15(3):R46](https://doi.org/10.1186/gb-2014-15-3-r46)*
...
- **Read extraction by taxon.** Tools such as `KrakenTools` (e.g. `extract_kraken_reads.py`) let you pull out only the reads assigned to your species or taxon of interest, or conversely remove reads assigned to a known contaminant (for example, human host reads).
...
![](./assets/04.reference_based_depletion.svg)
...
For this lesson we will use two tools:
...
- The FASTQ file of the query sample. Use the full path to the trimmed and filtered FASTQ file that you generated in [Lesson 3 - Read pre-processing](./03.preprocess.md).
- A Kraken2 database. The Kalamari database that we will be using today is located on the CVMFS file system at `~/data/ref/kalamari`
...
??? success "Solution"
    ...
    The following commands will create a new directory called `kraken2`, then run Kraken2 with default settings and place the results in the new `kraken2` directory. **Remember** to substitute your sample name for `sampleXX`.

    ```bash
    mkdir -p kraken2

    kraken2 \
        --db ~/data/ref/kalamari \
        --report kraken2/sampleXX.k2report \
        --output kraken2/sampleXX.k2_out.txt \
        --threads 4 \
        ~/filtered/sampleXX.filtered.fastq.gz
    ```
...
You should find two new files now present within the `kraken2` folder you created:
...
#### 4.3.1.1 (Optional) Experiment with different parameters
...
You can also open the report directly in the VS Code editor, either by navigating to it in the Explorer side bar, or using the `code` command:

```bash
code kraken2/sampleXX.k2report
```
