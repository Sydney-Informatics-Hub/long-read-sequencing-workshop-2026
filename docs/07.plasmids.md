...
### 7.2.3 Comparing `plassembler` and `flye` plasmid calls
...
- **Length and repeat content.** A `flye` contig flagged as a `repeat` (see the `assembly_info.txt` columns in [Lesson 6](./06.assembly.md)), or one whose length looks suspiciously similar to a known repetitive element (e.g. an rRNA operon), is more likely to be a collapsed repeat than a standalone plasmid.
...
### 7.3.3 Compare the `plassembler` and `flye` plasmid calls
...
For each candidate plasmid contig (whether it came from `flye`, `plassembler`, or both), use the checks discussed in [7.2.3](#723-comparing-plassembler-and-flye-plasmid-calls) to decide whether you think it's a genuine plasmid or likely an assembly artefact:
...
!!! hint "Discussion points"
    ...
    - Keep in mind that these checks are all circumstantial evidence, not proof. Later lessons on annotation and consensus assembly (e.g. [Lesson 8](./08.consensus.md)) will give you further tools to build confidence in your plasmid calls.

### 7.3.4 Add any new plasmids to your assembly
