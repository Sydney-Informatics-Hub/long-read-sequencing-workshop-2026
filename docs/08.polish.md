...
## 1. What is polishing, and why does it matter?
...
There are two broad approaches to polishing: long-read-only methods, which try to correct errors using nothing but the long-read data itself, and short-read polishing, where additional short-read sequencing of the same genome is used to complement and correct the long-read assembly. In this training we will use a long-read-only tool — medaka.

### 1.1. Should you polish with short reads too?

This training polishes assemblies using long reads alone, which should be enough if recent flow cell chemistries and basecalling models were used to create the data. If you want to push accuracy further, though, a common approach is to also polish with short reads. An example of a workflow could be: 
...
#### 2.1.1. Which draft assembler does medaka expect?
...
- `-m` — the medaka model to use. See ["Choosing the right model" (Section 2.1.3)](#213-choosing-the-right-model) for how to pick this.
...
To choose an appropriate model, you need to know how the reads were generated, including:

- the pore and sequencing chemistry
- the sequencing device
- the basecaller
- the basecalling accuracy model
- and, where relevant, the basecaller version

Medaka model names generally pack several of these into one string, separated by underscores. For example, an older-style name like `r941_prom_high_g303` breaks down as:

| Component | Meaning |
|---|---|
| `r941` | R9.4.1 pore and chemistry |
| `prom` | PromethION sequencing device |
| `high` | High-accuracy basecalling model |
| `g303` | Guppy version 3.0.3, with the dots removed |

The exact naming convention varies between older and newer medaka releases, covered separately below. Older models may use names such as `r941_prom_high_g303`, whereas newer models commonly use `r941_min_hac_g507` or `r1041_e82_400bps_sup_v5.2.0`. In newer model names:

- `fast` / `hac` / `sup` indicate a fast / high-accuracy / super-accuracy basecalling model, in place of the older `high`
- `r1041` indicates R10.4.1 chemistry
- `e82` identifies the associated chemistry/pore configuration
- `260bps` / `400bps` identifies the translocation speed, where applicable
- the final component identifies the basecaller release or model version

##### 2.1.3.1. Recent basecallers
...
##### 2.1.3.2. Older basecallers

If automatic model selection isn't successful, you can specify a medaka model directly with `medaka_consensus`'s `-m` flag:
...
Medaka's own model names are short and self-contained — no basecaller software prefix, no explicit kHz sampling-rate tag, no modified-base suffix. For example, here is a list of models available in `medaka 2.2.2`:
...
Beyond the pore type (`r941`, `r103`, `r104`, `r1041`) and device/chemistry components already covered above, most entries also carry a chemistry code (`e81`/`e82` — absent from `r103_*` and from the `r941_min_*`/`r941_prom_*`/`r941_sup_plant_g610` entries) and, on r1041 entries only, a translocation speed (`260bps`/`400bps` — the one pore generation that actually offered both). Version tags follow the same older/newer split as the model-type vocabulary: current models use a semantic version (`v4.0.0` through `v6.0.0`) or a Guppy-derived release tag (`g507`, `g514`, `g610`, `g615`, `g632`, `g5015`); older models use the Guppy release number with the dots removed.

!!! tip "Can't find the model you need?"
    If the exact model for your basecaller/version combination isn't in the list, check [medaka's changelog](https://github.com/nanoporetech/medaka/blob/master/CHANGELOG.md) to see which medaka version first shipped it — you may just need a newer medaka install. Failing that, the closest match for the same pore, chemistry, and accuracy tier (`fast`/`hac`/`sup`) is a reasonable fallback, ideally from the nearest lower version tag rather than a higher one.
...
1. Work out which medaka model to use to polish your sample, using [the samples information](00.samples.md) together with the model information in [Section 2.1.3, Older basecallers](#2132-older-basecallers) (we'll use `medaka 1.3.3` for this exercise — we checked medaka's changelog/release notes for you, to find which versions of medaka you could use). Confirm whether that model is actually available by listing the models (`medaka tools list_models`) — is it there?
...
medaka_consensus \
    ...
    -b 50
```
3. Run QC (QUAST + BUSCO) on the polished assembly — the same commands as the Assembly QC module, just pointed at `medaka/consensus.fasta` instead of the draft assembly, writing into `assembly_qc/quast/polished` and `assembly_qc/busco/polished` alongside the draft's own `assembly_qc/quast/draft` and `assembly_qc/busco/draft`.
4. Run MultiQC on the whole `assembly_qc` folder to compare the draft and polished reports side by side, then answer the following questions:
...
nanoporetech/medaka: Neural-network consensus polishing and variant calling for Oxford Nanopore sequencing data. Accessed September 17, 2026. https://github.com/nanoporetech/medaka

Medaka vs Dorado polish | Ryan Wick’s bioinformatics blog. Accessed September 17, 2026. https://rrwick.github.io/2025/02/07/dorado-polish.html

FAQ: Can we polish the assembly with long reads too? / Chloroplast genome assembly / Assembly. Accessed September 17, 2026. https://training.galaxyproject.org/training-material/topics/assembly/tutorials/chloroplast-assembly/faqs/polish_longreads.html

medaka/CHANGELOG.md at master · nanoporetech/medaka. Accessed September 17, 2026. https://github.com/nanoporetech/medaka/blob/master/CHANGELOG.md