...
Medaka model names generally pack several of these into one string, separated by underscores. For example, `r941_min_high_g360` breaks down as:

| Component | Meaning |
|---|---|
| `r941` | R9.4.1 pore and chemistry |
| `min` | MinION sequencing device |
| `high` | High-accuracy basecalling model |
| `g360` | Guppy version 3.6.0, with the dots removed |

The exact naming convention varies between older and newer medaka releases, covered separately below. Older models may use names such as `r941_min_high_g360`, whereas newer models commonly use `r941_min_hac_g507` or `r1041_e82_400bps_sup_v5.2.0`. In newer model names:
